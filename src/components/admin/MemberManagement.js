...
const MemberManagement = ({ members, setMembers, clubs }) => {
  ...
  useEffect(() => {
    ...
    const fetchClubs = async () => {
      try {
        ...
        // File: src/components/admin/MemberManagement.js (Line 54)
        // Mục đích: Admin lấy danh sách tất cả CLB để map clubIds với club names khi hiển thị thông tin user
        // Method: GET
        // Endpoint: /clubs
        // Headers: Authorization: Bearer {token} (optional)
        // Response: { code: 1000|0, result: Array<{ clubId, clubName, ... }> }
        // Response format: Array of club objects với clubId và clubName để tạo clubsMap
  ...
  useEffect(() => {
    ...
    const fetchUsers = async () => {
      ...
      try {
        ...
        // File: src/components/admin/MemberManagement.js (Line 110)
        ...
        //   code: 1000|0, 
        //   result: { 
        //     content: Array<{ userId, fullName, email, studentCode, clubIds, active, ... }>,
        //     totalPages: number,
        //     totalElements: number
        //   }
        // }
        // Response format: Paginated response với content array, totalPages, totalElements
  ...
  const handleDelete = async (member) => {
    ...
    try {
      // ========== API CALL: DELETE /users/{id} - Delete User (Deactivate) ==========
      // File: src/components/admin/MemberManagement.js (Line 282)
      ...
      // Response: { code: 1000|0, message: "Success message" }
      // Response format: Success message khi xóa thành công
      // Lưu ý: Đây là soft delete, user không bị xóa hoàn toàn mà chỉ bị đánh dấu inactive
